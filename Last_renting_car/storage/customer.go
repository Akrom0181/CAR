...
func (c *customerRepo) Create(customer models.Customer) (string, error) {
	...
	query := `insert into customers(id,first_name,last_name,gmail,phone,is_blocked,created_at) values($1,$2,$3,$4,$5,$6,CURRENT_TIMESTAMP)`

	_, err := c.db.Exec(query, id.String(), customer.FirstName, customer.LastName, customer.Gmail, customer.Phone, customer.Is_Blocked, customer.CreatedAt)
	if err != nil {
		return "error:", err
	}
...
func (c *customerRepo) GetAll(search string) (models.GetAllCustomersResponse, error) {
	var (
...
func (c *customerRepo) GetByIDPhoneName(id, first_name, last_name, phone string) (models.Customer, error) {
	customer := models.Customer{}

	if err := c.db.QueryRow(`select id,first_name,last_name,gmail,phone,is_blocked from customers where id = $1`, 
	id, first_name, last_name, phone).Scan(
		&customer.Id,
		&customer.FirstName,
		&customer.LastName,
		&customer.Gmail,
