...
func (c *customerRepo) Create(ctx context.Context, customer models.Customer) (string, error) {
	...
	_, err := c.db.Exec(context.Background(), query, id.String(),
	customer.FirstName,
	customer.LastName, 
	customer.Gmail, 
	customer.Phone, 
...
func (c *customerRepo) GetByID(ctx context.Context, id string) (models.Customer, error) {
	...
	if err := c.db.QueryRow(context.Background(), `select id, first_name, last_name, gmail, phone, is_blocked, created_at, updated_at from customers where id = $1`, id).Scan(
		&customer.Id,
		&customer.FirstName,
		&customer.LastName,
		&customer.Gmail,
