...
func (h Handler) GetAvailableCars(c *gin.Context) {
	...
	request.Page = page
	request.Limit = limit
	cars, err := h.Services.Car().GetAvailableCars(context.Background(), request)
	if err != nil {
		handleResponse(c, "error while getting cars", http.StatusBadRequest, err.Error())
		return
	}
...
func (h Handler) GetAllCars(c *gin.Context) {
	...
	request.Page = page
	request.Limit = limit
	cars, err := h.Store.Car().GetAll(context.Background(), request)
	if err != nil {
